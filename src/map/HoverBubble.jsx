...
const Hover = props => {
  ...
  return (  
    <div
      ...
      style={style}>
      {colocated > 0 ?
        <>{node.geometry.label ? node.geometry.label : node.properties.title} <span className="p6o-map-hover-colocated">{colocated+1} items</span></> : <>{node.geometry.label ? node.geometry.label : node.properties.title}</> 
      }
